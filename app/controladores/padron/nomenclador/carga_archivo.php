...
if (!empty($_FILES['archivo'])) {
	foreach ($_FILES['archivo']['name'] as $orden => $nombre) {
		...
		if ($_FILES['archivo']['error'][$orden] == 0 && move_uploaded_file ($_FILES['archivo']['tmp_name'][$orden] , '../../../upload/nomenclador/' . date("m") . "-N-" . $grupo ."_" . $uniqueid .".txt")) {
			$subidas[] = $nombre;
			...
			$sql = "
				...
				values (
					...
					, '" . $nombre . "' 
					, '" . date("m") . "-N-" . $grupo ."_" . $uniqueid . ".txt" . "' ) ";
			$res = pg_query ($sql);
			if (! $res) {
				unlink ("../upload/nomenclador/" . date("m") . "-N-" . $grupo . "_" . $uniqueid .".txt");
				echo pg_last_error();
			} else {
	...
	$("#submit").click(function(event){
		...
		switch (validacion_archivo) {
			...
			case 1 :
				...
				request.open("post" , "modulos/padron/tab-nomenclador/carga_archivo.php");
...
<div class="formulario-subida-datos">
	<form action="" method="post" enctype="multipart/form-data">
		<table>
			<thead>
				<tr>
					<th colspan="2">Ruta de archivo - Nomenclador</th>
				</tr>
			</thead>
			<tbody>
				<tr>
...
<p class="ui-state-highlight" style="border-radius: 3px;">
	<span class="ui-icon ui-icon-info" style="float: left; margin-right: .3em;"></span>Seleccione la ruta al archivo de <span class="nombre_padron">nomencladores</span> dentro de su ordenador. Recuerde respetar la estructura de datos y cargar el padr&oacute;n de comprobantes previamente.
</p>