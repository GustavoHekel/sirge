...
class Bdd {
	...
	public function aquery($sql, $params = array()) {
		...
		if ($this->_query = $this->_pdo->prepare($sql)) {
			$i = 1;
			if (count($params)) {
				foreach ($params as $nombreCampo => $valor) {
					if (is_int($valor)) {
						$this->_query->bindValue(':' . $nombreCampo, $valor, PDO::PARAM_INT);
					} else {
						$this->_query->bindValue(':' . $nombreCampo, $valor, PDO::PARAM_STR);
					}
					$i++;
				}
			}
	...
	public function faquery($file, $params = array(), $ajax = TRUE) {
		$clase = debug_backtrace()[1]['class'];
